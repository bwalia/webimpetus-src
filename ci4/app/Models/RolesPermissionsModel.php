...
class RolesPermissionsModel extends Model
{
    ...
    protected $table            = 'roles__permissions';
    ...
    public function getRowsByRole($uuid = false)
    {
        if($uuid === false){
            return [];
        }else{
            return $this->getWhere(['role_id' => $uuid])->getResultArray();
        }   
    }
    ...
    public function deleteDataByRole($id)
    {
        $query = $this->db->table($this->table)->delete(array('role_id' => $id));
        return $query;
    }
