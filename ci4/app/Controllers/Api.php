...
class Api extends BaseController
{
    ...
    public function webpages($customer_id = false)
    {
        ...
        if (count($webPagesId)) {
            $webpages = $this->cmodel->where("status", 1)->whereIn('id', $webPagesId)->orderBy("publish_date", 'ASC')->get()->getResult();
            if ($webpages) {
                $webPageList = [];
    ...
    public function addCustomer()
    {
        if (!empty($this->request->getPost('company_name')) && !empty($this->request->getPost('acc_no')) && !empty($this->request->getPost('uuid_business'))) {
            ...
            $response_data['status'] = 'error';
            $response_data['msg']    = 'You must specify the Business UUID, Company name and Customer Account number.!!';
            echo json_encode($response_data);
            die;
        }
    }
    ...
    public function updateCustomer()
    {
        if (!empty($this->request->getPost('company_name')) && !empty($this->request->getPost('acc_no')) && !empty($this->request->getPost('uuid')) && !empty($this->request->getPost('uuid_business'))) {
            ...
            $response_data['status'] = 'error';
            $response_data['msg']    = 'uuid, You must specify the Business UUID, Company name and Customer Account number.!!';
            echo json_encode($response_data);
            die;
        }
    }
    ...
    public function timeslips($ubusiness_id = "")
    {
        ...
        } else if (!empty($list_monthpicker)) {
            $arr2["MONTH(CAST(FROM_UNIXTIME(slip_start_date) as date))"] = $list_monthpicker;
            //$arr2['slip_start_date <='] = $submitted_time2;

        } else if (!empty($list_yearpicker)) {
