...
// Prepare statement for incidents
$incidentSql = "
...
$incidentStmt = $mysqli->prepare($incidentSql);

if (!$incidentStmt) {
    die("Prepare failed for incidents: " . $mysqli->error);
}
...
for ($i = 1; $i <= 1000; $i++) {
    ...
    $uuidBusinessId = 'BUS-' . str_pad(rand(1, 10), 3, '0', STR_PAD_LEFT);
    $uuidBusinessId = "329e0405-b544-5051-8d37-d0143e9c8829"; 
    ...
    $assignedTo = rand(1, 20);
    $reporterId = rand(1, 50);
    $customerId = rand(1, 100);
    $relatedKbId = rand(1, 50);

    $incidentStmt->bind_param(
        'ssssssssiiissssssiss',
        $uuid,
        $uuidBusinessId,
        $title,
        $description,
    ...
    if (!$incidentStmt->execute()) {
        echo "Error inserting incident $i: " . $incidentStmt->error . "\n";
    }
...
// Prepare statement for knowledge base
$kbSql = "
...
$kbStmt = $mysqli->prepare($kbSql);

if (!$kbStmt) {
    die("Prepare failed for knowledge base: " . $mysqli->error);
}
...
for ($i = 1; $i <= 1000; $i++) {
    ...
    if (!$kbStmt->execute()) {
        echo "Error inserting KB article $i: " . $kbStmt->error . "\n";
    }
...
// Summary
$result = $mysqli->query("SELECT COUNT(*) as count FROM incidents");
$row = $result->fetch_assoc();
$incidentCount = $row['count'];

$result = $mysqli->query("SELECT COUNT(*) as count FROM knowledge_base");
$row = $result->fetch_assoc();
$kbCount = $row['count'];
