...
if [ $clusterName == "k3s2" ] || [ $clusterName == "k3s3" ] || [ $clusterName == "k3s6" ] || [ $clusterName == "k3s10" ]; then
   ...
   helm upgrade -i wsl-$targetEnv ./devops/workerra-ci-chart -f devops/workerra-ci-chart/values-$targetEnv-$clusterName.yaml --set-string targetImage="bwalia/$IMAGE_NAME" --set-string targetImageTag="$IMAGE_TAG" --namespace $targetNs --create-namespace
