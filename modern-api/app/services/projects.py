...
from app.schemas.projects import Project as ProjectSchema
...
class ProjectService(BaseService):
    ...
    def list_projects(
        ...
        *,
        ...
        data = [ProjectSchema.model_validate(item, from_attributes=True) for item in items]
        page = cursor.apply(data)
        return PaginatedResponse[ProjectSchema](data=page.items, meta=page.meta, links=page.links)

    def create_project(self, payload: ProjectCreate, *, idempotency_key: str | None) -> ServiceResult:
        self._ensure_scope("crm.write")
        fingerprint = payload.model_dump_json()
        self._assert_idempotency(idempotency_key, fingerprint)

        instance = Project(**payload.model_dump(), tenant_id=self.principal.tenant_id)
        ...
        return ServiceResult(data=ProjectSchema.model_validate(instance, from_attributes=True), etag=etag)

    def get_project(self, project_id: str) -> ServiceResult:
        self._ensure_scope("crm.read")
        instance = self.repo.get(self.principal.tenant_id, project_id)
        if not instance:
            raise KeyError("Project not found")
        etag = self._compute_etag(instance.version, instance.updated_at)
        return ServiceResult(
            data=ProjectSchema.model_validate(instance, from_attributes=True),
            etag=etag,
