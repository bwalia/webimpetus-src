describe(`Workstation Categories test on ${Cypress.env("TARGET_ENV")} environment`, () => {    
    ...
    cy.wait(1000);    
    cy.get('select[id="uuid"]').select(`Cypress User ${randomString}`, {force: true});
    cy.get('input[name="name"]').type(`Cypress Category ${randomString}`);
    ...
    cy.get(`tr:contains('Cypress Category ${randomString}')`).should("contain", "Updated");

    // Verifying the search filter
    cy.contains('label', 'Search:').type(`${randomString}`)
    cy.wait(1000)
    cy.get(`tr:contains('Cypress Category ${randomString}')`).should('be.visible');

    cy.contains('label', 'Search:').type("xyz")
    cy.wait(1000)
    cy.get(`tr:contains('Cypress Category ${randomString}')`).should('not.be.visible');
