describe(`Workstation Sprints test on ${Cypress.env("TARGET_ENV")} environment`, () => {    
    ...
    cy.get('input[name="sprint_name"]').type(`Cypress Sprint ${randomString}`);
    ...
    cy.get(`tr:contains('Cypress Sprint ${randomString}')`).should("contain", "Updated");

    // Verifying the search filter
    cy.contains('label', 'Search:').type(`${randomString}`)
    cy.wait(1000)
    cy.get(`tr:contains('Cypress Sprint ${randomString}')`).should('be.visible');

    cy.contains('label', 'Search:').type("xyz")
    cy.wait(1000)
    cy.get(`tr:contains('Cypress Sprint ${randomString}')`).should('not.be.visible');
